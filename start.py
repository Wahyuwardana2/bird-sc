...
def list_items():
    ...
    # Mengumpulkan tipe item dan meminta harga
    prices = {}
    item_types = set(item['type'] for item in minted_items)  # Set untuk mendapatkan tipe unik

    print("Mengumpulkan harga untuk setiap tipe item...")
    for item_type in item_types:
        price = float(input(f"Masukkan harga untuk tipe {item_type.capitalize()}: "))
        prices[item_type] = price  # Simpan harga untuk tipe ini

    # Lakukan listing untuk setiap minted item
    for item in minted_items:
        listing_url = listing_url_template.format(id=item['id'])
        payload = {"price": prices[item['type']]}  # Ambil harga dari dictionary
...
def unlist_items():
    ...
    # Lakukan unlisting untuk setiap listed item
